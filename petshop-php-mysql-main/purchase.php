...
$result_all_items = mysqli_query($link, $query_all_items);
...
if (array_key_exists("buyItem", $_POST)) {
  ...
  $error = handleBuyItem($_POST["buyItem"], $amount, $link);
}

function handleBuyItem($id_buy, $amount_buy, $link_buy) {
  $query_validate_stock = "select stock from items where id=".$id_buy.";";
  if ($result_stock = mysqli_query($link_buy, $query_validate_stock)) {
    ...
    if (mysqli_query($link_buy, $query_buy_item)) {
      header("Refresh:0");
      return "";
    }
    return "sql error";
...
if (isset($_GET['search'])) {
  ...
  $result_search = mysqli_query($link, $search_string);
